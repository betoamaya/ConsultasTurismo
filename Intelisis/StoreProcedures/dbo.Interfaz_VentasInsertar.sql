...
BEGIN
    ...
    BEGIN
        ...
        VALUES
        ('V.ESP.GRAVADO', @Precio, @TasaImpuesto);
    END;
    ELSE
    BEGIN
        INSERT INTO @T_PartidasVtas
    ...
    SELECT @Almacen = (CASE
                           WHEN RTRIM(@Empresa) = 'TUN' THEN
                               'TUN_VE'
                           WHEN RTRIM(@Empresa) = 'TSL' THEN
                               'TSL_VE'
                           ELSE
                               NULL
                       END
    ...
    BEGIN
        SET @bMovValido = 1;
        IF (@Concepto <> 'ENTRADAS A PARQUES' AND @Concepto <> 'EXTRA PAQUETES')
        BEGIN
            SET @sError
                = 'Concepto no valido. El concepto se valida de acuerdo al movimiento y usuario indicado. '
                  + 'Por favor, indique un Concepto valido para esta combinación de movimiento y usuario.';
    ...
    BEGIN
        ...
        BEGIN
            ...
            SET Articulo = 'TI-10',
                TasaImpuesto = 11;
        END;
        ELSE IF (@Concepto = 'T.INDUSTRIAL 15%')
        BEGIN
            SELECT @Condicion = 'Credito',
                   @Impuesto1 = 16;
            UPDATE @T_PartidasVtas
    ...
    BEGIN
        SET @bMovValido = 1;
        IF (
               @Mov IN ( 'Factura VE Total', 'INE VE TOTAL' )
               AND @Concepto <> 'VIAJES ESPECIALES'
           )
           OR (
                  @Mov = 'Ingreso Paquetes'
        ...
        BEGIN
            ...
            FROM @T_MovRelacionados AS B
                LEFT JOIN Cxc AS C
                    ON B.ID = C.ID;
            IF EXISTS (SELECT CxcID FROM @T_ComparacionAnticipos WHERE CxcID IS NULL)
            BEGIN
                SET @sError
                    = 'Uno o mas identificadores de anticipos indicados, no fueron encontrados (IDs no encontrados ';
                SET @sError = @sError +
                              (
                                  SELECT TOP 1
                                      (
            ...
            BEGIN
                SET @sError
                    = 'El importe de uno o mas anticipos, no corresponde con los saldos (IDs que no coinciden ';
                SET @sError = @sError +
                              (
                                  SELECT TOP 1
                                      (
    ...
    BEGIN
        ...
        BEGIN
            ...
            SELECT @RegresoID,
                   ...
                   @TipoPago;
        END;
        IF (@sreferencia IS NOT NULL AND @Condicion = 'Credito')
        BEGIN
            INSERT INTO VentaCobro
            (
                ID,
    ...
    BEGIN
        BEGIN TRY
            ...
            EXEC dbo.spCFDFlex @Estacion = 881,
                               @Empresa = @Empresa,
                               @Modulo = 'VTAS',
                               @ID = @RegresoID,
                               @Estatus = 'CONCLUIDO',
        ...
        IF
        (
            SELECT ISNULL(CFDFlexEstatus, '') FROM dbo.Venta WHERE ID = @RegresoID
        ) <> 'CONCLUIDO'
        BEGIN
            SET @sError
    ...
    SELECT v.ID,
           ...
           v.Estatus,
           CASE
               WHEN c.UUID = NULL THEN
                   RTRIM(ISNULL(@sError, ''))
               ELSE
                   RTRIM(v.CFDFlexEstatus)
